...
(function($){
	...
	var Validato = function(ele, options){
		...
		// 显示错误的方式，用户没设置的话，则默认layer
		Validato.options.errorPopType = ( options && options.errorPopType != undefined ) ? options.errorPopType : ['layer'];
		// var t = $.extend(Validato.errorPopType, Validato.addErrorPop);
		// C.log(Validato.options.addErrorPop);return false;
		// console.log(Validato.errorPopType);
		// console.log(Validato.options);
		// return false;

		// 
		// 
		if ( Validato.options.errorPopType.indexOf('layer') > -1 ) {
	...
	Validato.prototype = {
		...
		validating:function(){
			...
			if ( Validato.options == null || Validato.options === undefined ) {
				...
				allInputObj.each(function(index, item){
					...
					var crtBoxVal = Validato.getElementVal(item);
				...
				for (var i in itemRules) {
					...
					var crtBoxVal =  Validato.getElementVal(crtItemName); // 当前等待验证的表单值
		...
		clearError:function(itemName){
			// var selector = 
			if ( this.options.errorPopType.indexOf('default') > -1 ) {
				// 如果有追加式错误提示的话，则去除错误
		...
		**/
		// 必须与某个值相同
		same:function(val, selector, errorTips){
			// 
			if( val != this.jqObjectize(selector).val() ){
			...
			if ( params.data ) {
				...
				for( var key in params.data ){
					...
					if ( ele === undefined ) {
						...
						// 获取元素的值
						finalData[itemName] = this.getElementVal(params.data[itemName]);
						// 
		...
		getElementVal:function(selector){
			...
			var elementVal = null;
			var tagName = this.jqObjectize(selector).prop('tagName').toLowerCase();
			// C.log( tagName );return false;
			var tagType = this.jqObjectize(selector).prop('type').toLowerCase();
			// radio
			if ( tagType === 'radio' ) {
				elementVal = this.jqObjectize(selector).find(':checked').val();
			} else if ( tagType === 'checkbox' ) {
				elementVal = new Array();
				for( var checkItem in this.jqObjectize(selector).find(':checked') ){
					elementVal.push($(checkItem).val());
				}
			} else if( tagName == 'input' || tagName == 'textarea' ){
				elementVal = this.jqObjectize(selector).val();
			} else if( tagName == 'select' ){
				elementVal = this.jqObjectize(selector).find('option:selected').val();
			}
			return elementVal;
		},
		// 获取jquery形式对象
		jqObjectize:function(itemName){
			...
			if ( itemName.indexOf('.') > -1 || itemName.indexOf('#') > -1 ) {
				// console.log(this.crtForm);
				return this.crtForm.find(itemName);
			} else {
				// console.log(this.crtForm);
				return this.crtForm.find('[name=' + itemName + ']');
			}
		},
		// Validato主体结束
	}
